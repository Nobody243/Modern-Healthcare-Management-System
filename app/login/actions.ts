  ...
  try {
    ...
    if (role === 'admin') {
      ...
      const isValidPassword = user && (
        password === user.AD_PWD ||
        (user.AD_PWD?.startsWith('$2') ? await verifyPassword(password, user.AD_PWD) : false) ||
        (isDemoAccount && (password === 'demo123' || password === 'admin'))
      );
      ...
      const isValidPassword = user && (
        password === user.DOC_PWD ||
        (user.DOC_PWD?.startsWith('$2') ? await verifyPassword(password, user.DOC_PWD) : false) ||
        (isDemoAccount && (password === 'demo123' || password === 'doctor123'))
      );
      ...
      const user = await queryOne(
        `SELECT PAT_ID, PAT_FNAME, PAT_LNAME, PAT_EMAIL, PAT_NUMBER
      ...
      if (!user) {
        ...
        return { error: 'Patient account not found. Please check your email or use a demo account.' };
      }
