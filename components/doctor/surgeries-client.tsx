        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4">
          ...
          >
            ...
            <p className="text-xs sm:text-sm text-muted-foreground leading-relaxed">
              Surgical operative schedules, assigned theater bookings, and postoperative charts
            </p>
          </motion.div>
          <motion.div
            initial={{ opacity: 0, x: 20 }}
                      ...
                      <div>
                        <CardHeader className="flex flex-row items-start justify-between pb-3 gap-3">
                          <div className="flex items-start gap-3 min-w-0 flex-1">
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              <div className="text-xs text-muted mt-1.5 flex flex-wrap items-center gap-2">
                                <span className="font-bold text-heading">Surgery #{surgery.SURG_NUMBER}</span>
                                {surgery.SURG_PAT_NUMBER && (
                                  <>
                                    <span className="text-muted-foreground">•</span>
                                    <span className="table-id-link font-mono font-bold whitespace-nowrap">{surgery.SURG_PAT_NUMBER}</span>
                                  </>
                                )}
                                <span className="text-muted-foreground">•</span>
                          ...
                          <div className="flex items-center gap-1.5 shrink-0">
                            ...
                            <Button
                              ...
                              className="table-action-delete hover-lift cursor-pointer h-8 w-8 shrink-0"
                        ...
                        <CardContent className="pt-1 pb-3">
                          <div className="grid grid-cols-3 gap-2 text-xs">
                            <div className="bg-muted/40 dark:bg-muted/20 p-2.5 rounded-xl border border-border">
                              <span className="text-muted block text-[10px] uppercase font-bold tracking-wider">Date</span>
                              <span className="font-bold text-heading mt-0.5 block truncate">{formatDate(surgery.SURG_DATE)}</span>
                            </div>
                            <div className="bg-muted/40 dark:bg-muted/20 p-2.5 rounded-xl border border-border">
                              <span className="text-muted block text-[10px] uppercase font-bold tracking-wider">Type</span>
                              <span className="font-bold text-heading mt-0.5 block truncate">{surgery.SURG_TYPE || 'General'}</span>
                            </div>
                            <div className="bg-muted/40 dark:bg-muted/20 p-2.5 rounded-xl border border-border">
                              <span className="text-muted block text-[10px] uppercase font-bold tracking-wider">Duration</span>
                              <span className="font-bold text-heading mt-0.5 block truncate">{surgery.SURG_DURATION || '—'}</span>
                            </div>
                          </div>
                          {surgery.SURG_NOTES && (
                            <div className="mt-3 p-3 rounded-xl bg-muted/40 dark:bg-muted/20 border border-border text-xs">
                              <span className="font-bold text-heading block text-[10px] uppercase tracking-wider mb-0.5">Notes:</span>
                              <p className="text-muted line-clamp-2">{surgery.SURG_NOTES}</p>
                            </div>
                          )}
                        </CardContent>
                      </div>
