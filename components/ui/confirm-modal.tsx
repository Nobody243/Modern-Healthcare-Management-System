'use client';
import { motion, AnimatePresence } from 'framer-motion';
import { AlertTriangle, Loader2, X, Trash2, LogOut } from 'lucide-react';
import { Button } from './button';

interface ConfirmModalProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: () => Promise<void> | void;
  title: string;
  description: string;
  confirmText?: string;
  cancelText?: string;
  variant?: 'danger' | 'warning' | 'primary';
  isLoading?: boolean;
  icon?: 'trash' | 'logout' | 'alert';
}

export function ConfirmModal({
  isOpen,
  onClose,
  onConfirm,
  title,
  description,
  confirmText = 'Confirm',
  cancelText = 'Cancel',
  variant = 'danger',
  isLoading = false,
  icon = 'trash',
}: ConfirmModalProps) {
  if (!isOpen) return null;

  const IconComponent = icon === 'logout' ? LogOut : icon === 'alert' ? AlertTriangle : Trash2;

  const iconBg =
    variant === 'danger'
      ? 'bg-kpi-danger-subtle text-kpi-danger'
      : variant === 'warning'
      ? 'bg-kpi-warning-subtle text-kpi-warning'
      : 'bg-kpi-primary-subtle text-kpi-primary';

  const confirmBtnClass =
    variant === 'danger'
      ? 'btn-danger'
      : variant === 'warning'
      ? 'bg-gradient-to-r from-[rgb(var(--kpi-warning-from))] to-[rgb(var(--kpi-warning-to))] text-white shadow-lg'
      : 'btn-primary';

  return (
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          {/* Backdrop */}
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            onClick={isLoading ? undefined : onClose}
            className="fixed inset-0 bg-background/80 backdrop-blur-sm transition-opacity"
          />

          {/* Modal Box */}
          <motion.div
            initial={{ opacity: 0, scale: 0.95, y: 15 }}
            animate={{ opacity: 1, scale: 1, y: 0 }}
            exit={{ opacity: 0, scale: 0.95, y: 15 }}
            transition={{ type: 'spring', duration: 0.3, bounce: 0 }}
            className="relative w-full max-w-md bg-card border border-border rounded-2xl shadow-2xl overflow-hidden z-10 text-card-foreground"
          >
            {/* Top colored accent line */}
            <div
              className={`h-1.5 w-full bg-gradient-to-r ${
                variant === 'danger'
                  ? 'from-[rgb(var(--kpi-danger-from))] to-[rgb(var(--kpi-danger-to))]'
                  : variant === 'warning'
                  ? 'from-[rgb(var(--kpi-warning-from))] to-[rgb(var(--kpi-warning-to))]'
                  : 'from-[rgb(var(--kpi-primary-from))] to-[rgb(var(--kpi-primary-to))]'
              }`}
            />

            <div className="p-6 space-y-4">
              {/* Header */}
              <div className="flex items-start gap-4">
                <div className={`p-3 rounded-xl border shrink-0 ${iconBg}`}>
                  <IconComponent className="w-6 h-6" />
                </div>
                <div className="flex-1">
                  <h3 className="text-lg font-bold text-heading">{title}</h3>
                  <p className="text-sm text-muted mt-1 leading-relaxed">{description}</p>
                </div>
                {!isLoading && (
                  <button
                    onClick={onClose}
                    className="text-muted hover:text-foreground transition-colors p-1 rounded-lg hover:bg-secondary cursor-pointer"
                  >
                    <X className="w-5 h-5" />
                  </button>
                )}
              </div>

              {/* Actions */}
              <div className="flex items-center justify-end gap-3 pt-4 border-t border-border">
                <Button
                  type="button"
                  variant="outline"
                  onClick={onClose}
                  disabled={isLoading}
                  className="btn-secondary"
                >
                  {cancelText}
                </Button>
                <Button
                  type="button"
                  onClick={onConfirm}
                  disabled={isLoading}
                  className={`${confirmBtnClass} font-semibold cursor-pointer min-w-[110px] flex items-center justify-center gap-2`}
                >
                  {isLoading ? (
                    <>
                      <Loader2 className="w-4 h-4 animate-spin" />
                      <span>Processing...</span>
                    </>
                  ) : (
                    <span>{confirmText}</span>
                  )}
                </Button>
              </div>
            </div>
          </motion.div>
        </div>
      )}
    </AnimatePresence>
  );
}
